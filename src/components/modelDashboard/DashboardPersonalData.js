...
export default function DashboardPersonalData ({hasDafault, data, id}){
    ...
    const getCitiesByCountry = async (countryCode) => {
        const username = "angeldev";
        const url = `http://api.geonames.org/searchJSON?country=${countryCode}&featureClass=P&maxRows=1000&username=${username}`;
        ...
        try {
            ...
            if (data.geonames.length > 0) {
                const citiesData = data.geonames.map(city => city.name)
